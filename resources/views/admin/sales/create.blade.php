                        ...
                        <table class="min-w-full text-sm" id="itemsTable">
                            <thead>
                                <tr class="border-b border-gray-200 text-left text-gray-600">
                                    ...
                                    <th class="py-2 pr-2 font-medium w-20">Qty</th>
                                    <th class="py-2 pr-2 font-medium w-28">Price</th>
                                    <th class="py-2 pr-2 font-medium w-28 text-right">Subtotal</th>
                    ...
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <select name="payment_method" id="payment_method" required
                                ...
                                <option value="credit" @selected(old('payment_method') === 'credit')>Credit (unpaid)</option>
    ...
    <script>
        ...
        function productOptions(selectedId) {
            ...
            PRODUCTS.forEach(function (p) {
                ...
                opts += '<option value="' + p.id + '" data-price="' + p.selling_price + '" data-stock="' + p.quantity + '" ' + sel + '>'
                     + p.name + ' (' + p.sku + ') — Stock: ' + p.quantity + '</option>';
            });
            return opts;
        }
        ...
        function addRow(productId, qty, price) {
            ...
            tr.innerHTML =
                '<td class="py-2 pr-2">' +
                    ...
                    '</select>' +
                    '<p class="stock-hint text-xs text-gray-500 mt-1"></p>' +
                '</td>' +
                '<td class="py-2 pr-2">' +
                ...
                '</td>' +
                '<td class="py-2 pr-2">' +
                    '<input type="number" step="0.01" min="0" name="items[' + rowCounter + '][price]" value="' + price + '" oninput="recalc()" required ' +
                           'class="w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500">' +
                '</td>' +
            ...
            if (productId) {
                onProductChange(tr.querySelector('select'), true);
            }
        ...
        function onProductChange(select, keepPrice) {
            ...
            if (stock) {
                ...
                hint.className = 'stock-hint text-xs mt-1 ' + (parseInt(stock) <= 0 ? 'text-red-600' : 'text-gray-500');
            } else {
                hint.textContent = '';
            }
            recalc();
