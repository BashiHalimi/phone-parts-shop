...
@section('content')
    ...
    <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div class="rounded-lg bg-white p-4 shadow-sm">
            ...
            <p class="mt-1 text-lg font-bold text-indigo-700">{{ number_format((float)($totals->total_total ?? 0), 2) }}</p>
        </div>
        <div class="rounded-lg bg-white p-4 shadow-sm">
                ...
                <table class="min-w-full text-sm">
                    <thead class="border-b border-gray-200">
                        <tr class="text-left text-gray-500">
                            ...
                            <th class="py-2 text-right font-medium">Sales Count</th>
    ...
    <div class="overflow-hidden rounded-lg bg-white shadow-sm">
        <div class="border-b border-gray-200 p-4">
            <h3 class="text-sm font-semibold uppercase tracking-wider text-gray-500">Individual Sales</h3>
        </div>
        <div class="overflow-x-auto">
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($sales as $sale)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3">
                                <a href="{{ route('admin.sales.show', $sale) }}" class="font-medium text-indigo-600 hover:underline">
                                    {{ $sale->invoice_no }}
                                </a>
                            </td>
                            <td class="px-4 py-3 text-gray-500">{{ $sale->created_at->format('Y-m-d H:i') }}</td>
                            <td class="px-4 py-3 text-gray-700">{{ $sale->customer?->name ?? 'Walk-in' }}</td>
                            <td class="px-4 py-3 text-right font-medium text-gray-800">{{ number_format((float)$sale->total, 2) }}</td>
                            <td class="px-4 py-3 text-right text-green-700">{{ number_format((float)$sale->paid, 2) }}</td>
                            <td class="px-4 py-3 text-right text-red-700">{{ number_format((float)$sale->balance, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500">No sales found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
